#! /bin/bash

if [[ $1 == "test" ]]
then
  PSQL="psql --username=postgres --dbname=worldcuptest -t --no-align -c"
else
  PSQL="psql --username=freecodecamp --dbname=worldcup -t --no-align -c"
fi

# Do not change code above this line. Use the PSQL variable above to query your database.

echo "$($PSQL "TRUNCATE teams, games")";
cat games.csv | while IFS="," read YEAR ROUND WINNER OPPONENT WINNER_GOALS OPPONENT_GOALS
do
  if [[ $WINNER != 'winner' ]]
  then
    # Insert Winning Team - Duplicates will not be inserted (but through an error) due to Unique constraint
    INSERT_WINNER="$($PSQL "INSERT INTO teams (name) VALUES('$WINNER')")";
    if [[ $INSERT_WINNER == 'INSERT 0 1' ]]
    then
      echo $WINNER inserted;
    fi
    # Insert Opponent Team - Duplicates will not be inserted (but through an error) due to Unique constraint
    INSERT_OPPONENT="$($PSQL "INSERT INTO teams (name) VALUES('$OPPONENT')")";
    if [[ $INSERT_OPPONENT == 'INSERT 0 1' ]]
    then
      echo $OPPONENT inserted;
    fi

    #Get Winner ID
    WINNER_ID="$($PSQL "SELECT team_id FROM teams WHERE name='$WINNER'")";
    #Get Opponent ID
    OPPONENT_ID="$($PSQL "SELECT team_id FROM teams WHERE name='$OPPONENT'")";

    #Insert Into Games
    INSERTED_GAME="$($PSQL "INSERT INTO games(year, round, winner_id, opponent_id, winner_goals, opponent_goals) VALUES($YEAR, '$ROUND', $WINNER_ID, $OPPONENT_ID, $WINNER_GOALS, $OPPONENT_GOALS)")";
    if [[ $INSERTED_GAME == 'INSERT 0 1' ]]
    then
      echo $WINNER vs $OPPONENT inserted;
    fi
  fi
done